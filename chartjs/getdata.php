...
//if(!$sidx) $sidx =1;
    ...
    $SQL="SELECT name, sum(kol) kol_dep, sum(deposit) sum_dep, round(sum(deposit)/sum(kol),2) average
...
for ($i = 0; $row = mysql_fetch_assoc($result); $i++) {
    ...
    $responce->rows[$i]['cell'] = array( $row['name'],$row['kol_dep'],$row['sum_dep'], $row['average']);
}
