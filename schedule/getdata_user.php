...
$SQL="select schedule.id, clients.id_platform, clients.platform, clients.name,
       clients.secret_phone, substring(schedule.date_call,1,10) date_call,
       CASE 
       ...
       when result like 'in process' then 'в процессе'
        ...
        clients.note, schedule.clients_id, 
        clients.phone
       from schedule       
		left join rtd on schedule.clients_id=rtd.clients_id
        left join clients on clients.id=schedule.clients_id
        where  clients.team=$team and (date_call =STR_TO_DATE('$today 00:00:00','%Y-%m-%d %H:%i:%s') or result not like  'done'
        and date_call < STR_TO_DATE('$today 00:00:00','%Y-%m-%d %H:%i:%s')) $where $filter
        ORDER BY  $sidx $sord";
...
for ($i = 0; $row = mysql_fetch_assoc($result); $i++) {
    ...
    $responce->rows[$i]['cell'] = array( $row[id_platform],$row[platform],$row[name],$row[secret_phone],$last[calldate],
        $row[date_call],  $row[task], $row[result], $row[note], $row[clients_id]);
}
