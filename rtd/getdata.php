...
{	
    ...
    $SQL=" select clients.id, clients.id_platform, clients.platform, clients.name, clients.birthday,
       clients.secret_phone, clients.mail, clients.team,
       CASE
       ...
       dep.date_rtd, dep.last_rtd, dep.sum_rtd, rtd.note, rtd.avg_income, rtd.field_activity, rtd.kind_activity, clients.phone, clients.documents
       from clients
...
for ($i = 0; $row = mysql_fetch_assoc($result); $i++) {
    ...
    $responce->rows[$i]['cell'] = array( $row[id_platform],$row[platform],$row[name],$row[note],$row[phone],$row[secret_phone],$row[mail],$row[team],$row[status],$last[calldate],
        $row[skype_date], $row[skype_name],$row[date_contact],
        $row[date_ftd],$row[amount_ftd], $row[date_rtd],$row[last_rtd], $row[sum_rtd], $row[birthday], $row[avg_income], $row[field_activity], $row[kind_activity],$row[documents]);
}
